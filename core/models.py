...
class TestElement(BaseTopicElement):
    ...
    def to_html(self):
        ...
        return u'<h2>{description}</h2><iframe src="http://127.0.0.1:8001/tutor/test_interactive/{test}/{level}/{FX}" frameborder="0" scrolling="yes" width="100%" height=500;"></iframe>'.format(description=self.description, test=self.test, level=self.difficulty, FX=str(' '.join(self.effect_files)))
